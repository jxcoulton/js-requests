...
// CODE HERE
const sayHelloBtnEvent = (e) => {
    e.target.style.backgroundColor = 'black'
    e.target.style.color = 'white'
}

sayHelloButton.addEventListener('mouseover', sayHelloBtnEvent)
...
// CODE HERE
const sayHelloBtnRevert = (e) => {
    e.target.style.backgroundColor = '#EFEFEF'
    e.target.style.color = 'black'
}

sayHelloButton.addEventListener('mouseout', sayHelloBtnRevert)
...
*/
const repeatParam = document.querySelector('#repeat-button')
...
repeatParam.addEventListener('click', repeatMyParam)
...
/*
    ...
    Write a function that makes a get request to 'http://localhost:3000/query-test', with a query of your choice
    on the end!
...
const queryRequest = () => {
    axios.get('http://localhost:3000/query-test/?name=bill')
...
/* 
    ...
    On each iteration of the loop, create a new p element. Set its
     textContent equal the string at the current index (i) and then append the
      new p element onto the document's body. 
*/
...
/*
    You are going to add the ability to POST to the server. You'll need
     ...
     Then you'll attach that function to the submit event on the form. We'll 
     be creating a list of foods. 
    ...
    Next, create an object called body inside the function. It should 
    have one key-value pair. The key should be newFood (make sure to match 
        the case and spelling exactly) and the value should be the value of
         the food input. 
